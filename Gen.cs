...
namespace Gen
{
    ...
    public class Play2
    {
        double[] sampleBufferL;
        double[] sampleBufferR;
        int ch, frame, n;
        public double speed { get; set; }
        public double phase { get; set; }
        double delta;
        ...
        public double[] val()
        {
            ...
            if (n >= frame - 1) return new double[2] {0, 0};

            output[0] = delta * sampleBufferL[n + 1] + (1.0 - delta) * sampleBufferL[n];
            output[1] = delta * sampleBufferR[n + 1] + (1.0 - delta) * sampleBufferR[n];
